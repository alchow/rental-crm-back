...
export function registerOutboxLifecycleRoutes(app: CommsApp): void {
  const listOutbox = createRoute({
    ...
    description:
      ...
      'Two landlord surfaces read the LIST rather than the single-row read ' +
      ...
      'and the statement rent-nudge trail. Owner/manager read it under the ' +
      'same principal rule as GET /comms/outbox/{id} — a landlord who may ' +
      'read one of their outbox rows may page all of them. Viewers are ' +
      'denied, and the dispatch/reconcile MUTATIONS are transport-only.',
    request: {
      params: AccountParam,
      query: z.object({
