...
const voidRow = async (kind: 'charges' | 'payments', id: string): Promise<void> => {
  const r = await api('POST', `/v1/accounts/${acct}/${kind}/${id}/void`, {
    token,
    body: { void_reason: 'rollup fixture void' },
  });
...
const mkTenancy = async (name: string, status_: string, start: string) => {
  const area = await post<{ id: string }>('/areas', {
    property_id: property.id,
    kind: 'unit',
    name,
  });
  return post<{ id: string }>('/tenancies', {
    area_id: area.id,
    start_date: start,
    status: status_,
  });
...
const p4 = await post<{ payment: { id: string } }>('/payments', {
  ...
  amount_cents: 7000,
  currency: 'USD',
  received_at: '2026-02-04T00:00:00.000Z',
  method: 'cash',
...
await post('/payments', {
  ...
  amount_cents: 4000,
  currency: 'USD',
  received_at: '2026-02-16T00:00:00.000Z',
  method: 'cash',
...
// T3: ended, with money — included when ended is requested so past-tenant
// debt remains actionable without changing the endpoint's legacy default.
...
// T5: ended with no money — must not make the unpaginated default grow forever.
const t5 = await mkTenancy('Unit R5', 'ended', '2025-01-01');
...
const getRollupResponse = async (qs = ''): Promise<{ as_of: string; data: RollupRow[] }> => {
  const suffix = `${qs}${qs ? '&' : '?'}as_of=2026-03-01`;
  const r = await api('GET', `/v1/accounts/${acct}/rent-rollup${suffix}`, { token });
  if (r.status !== 200) throw new Error(`rollup GET failed: ${r.status} ${JSON.stringify(r.body)}`);
  return r.body as { as_of: string; data: RollupRow[] };
};
const getRollup = async (qs = ''): Promise<RollupRow[]> => (await getRollupResponse(qs)).data;
...
await check('(1) exact numbers on the kitchen-sink tenancy', async () => {
  const response = await getRollupResponse();
  assertEq(response.as_of, '2026-03-01', 'as_of echo');
  ...
  assertEq(r1.rent_balance_cents, 68000, 'rent_balance');
  ...
  assertEq(r1.non_deposit_overdue_cents, 50000, 'overdue');
  assertEq(r1.non_deposit_due_today_cents, 12000, 'due_today');
  assertEq(r1.non_deposit_upcoming_cents, 6000, 'upcoming');
  assertEq(r1.deposit_owed_cents, 0, 'deposit_owed');
  assertEq(r1.deposit_held_cents, 30000, 'deposit_held');
});
...
await check('(2) zero-money tenancy gets a zero row with null currency', async () => {
  ...
  assertEq(r2.non_deposit_overdue_cents, 0, 'overdue');
  assertEq(r2.non_deposit_due_today_cents, 0, 'due_today');
  assertEq(r2.non_deposit_upcoming_cents, 0, 'upcoming');
  assertEq(r2.deposit_owed_cents, 0, 'deposit_owed');
  assertEq(r2.deposit_held_cents, 0, 'deposit_held');
...
await check('(3) PARITY: rollup equals GET /ledger for every returned tenancy', async () => {
  ...
  for (const row of rows) {
    ...
    assertEq(
      row.rent_balance_cents,
      ledger.totals.rent_balance_cents,
      `t=${row.tenancy_id} rent parity`,
    );
    assertEq(
      row.deposit_balance_cents,
      ledger.totals.deposit_balance_cents,
      `t=${row.tenancy_id} deposit parity`,
    );
    assertEq(
      row.unapplied_credit_cents,
      ledger.totals.unapplied_credit_cents,
      `t=${row.tenancy_id} credit parity`,
    );
...
await check(
  '(3b) currency edge: tenancy whose only charge is voided still reports it',
  async () => {
    const rows = await getRollup();
...
await check('(4) ended debt is opt-in and ended zero-money rows stay omitted', async () => {
  const rows = await getRollup();
  if (rows.some((r) => r.tenancy_id === t3.id || r.tenancy_id === t5.id)) {
    throw new Error('default active/holdover contract unexpectedly included an ended tenancy');
  }
  const ended = await getRollup('?status=active,holdover,ended');
  const r3 = ended.find((r) => r.tenancy_id === t3.id);
  if (!r3) throw new Error('ended debt missing from combined current + ended request');
  assertEq(r3.rent_balance_cents, 50000, 'ended rent_balance');
  if (ended.some((r) => r.tenancy_id === t5.id)) {
    throw new Error('ended zero-money tenancy leaked into requested set');
  }
...
await check('(5) bogus status value -> 400 with fieldErrors.status', async () => {
  ...
  const err = (
    r.body as { error: { code: string; details?: { fieldErrors?: Record<string, unknown> } } }
  ).error;
...
await check(
  ...
  async () => {
    ...
    if ((own.data as unknown[]).length < 2)
      throw new Error('own-account rpc returned too few rows');
  },
);
