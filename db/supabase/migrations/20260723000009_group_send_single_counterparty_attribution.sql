-- ----------------------------------------------------------------------------
-- Group-send journal attribution: a group with exactly one counterparty is
-- "with" that party.
--
-- complete_send journaled EVERY group send as party_type='unspecified'
-- (party_id null), so the FE's needs-attribution affordance asked "who was
-- this with?" for every move-in group text — an answer the function's own
-- cast assembly already names (the recipient snapshot / thread bindings carry
-- each member's party). First observed on the first live group MMS,
-- 2026-07-22.
--
-- RULE
-- After the cast is assembled, count cast members whose party_type is
-- tenant/vendor with a party_id: exactly ONE -> attribute it (party_type,
-- party_id, and its label become the journal row's "with"); zero or several
-- -> keep 'unspecified' + the joined-names label (a multi-tenant group has
-- no single honest counterparty). landlord_user members never attribute —
-- the landlord on a group thread is the delivery mechanism, not who the
-- record is about. Applies to both cast sources (recipient_snapshot and the
-- legacy bindings fallback) because it reads the assembled v_cast.
--
-- The function body below is the live definition (db/current-schema.sql)
-- with two additions: the v_cp_count/v_cp declarations and the attribution
-- block after the group label assembly. Non-group branches are byte-identical.
-- ----------------------------------------------------------------------------
