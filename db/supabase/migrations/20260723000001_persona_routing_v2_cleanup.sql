-- ============================================================================
-- Persona email routing v2, PR 6 — cleanup (plan §9.6.1)
-- ============================================================================
-- Three things, all deferred on purpose from the v2 stack
-- (20260722000001..3):
--
--   (A) DROP _capture_persona_inbound_before_reply_recovery — the dormant
--       pre-v2 classifier. 20260721000004 renamed the then-live
--       capture_persona_inbound to this name and wrapped it; 20260722000001
--       replaced the wrapper with the canonical coordinator, which does not
--       call it. Since then the function has been unreachable (EXECUTE was
--       revoked from every role including service_role in 20260721000004) and
--       would throw anyway if invoked — its body still targets the pre-claims
--       `on conflict (account_id, channel, address)` key that 20260722000002
--       dropped.
--       Reference check (verified at authoring time): grep of
--       db/supabase/migrations + api/src finds the name only in
--       20260721000004 (the rename + the superseded wrapper body that
--       20260722000001 replaced) and in comments/headers. No live function
--       body, route, or test calls it. It is not in the
--       db/test/check_definer_grants.sql allowlist, so dropping it cannot
--       trigger allowlist rot (capture_persona_inbound stays allowlisted).
--
--   (B) complete_send learns graceful duplicate-Message-ID handling —
--       re-created VERBATIM from the current head (20260722000002 §J) with
--       ONE addition: before stamping rfc822_message_id, look for a DIFFERENT
--       comm_outbox email row in the same account already holding the same
--       normalized Message-ID; if one exists, stamp NULL instead and RAISE
--       WARNING with both row ids. Completion must never fail on a duplicate
--       (a provider retry/reconcile echo re-reporting an id is an operational
--       fact, not an error) — the evidence stays on the first row, and the
--       degraded row simply does not participate in parent-probe dedupe.
--
--   (C) Message-ID uniqueness (deferred from the PR 1 review): with (B) in
--       place a UNIQUE index can no longer 500 the completion hot path, so —
--       when the existing data is collision-free — the non-unique
--       comm_outbox_email_msgid_idx from 20260722000001 §B is replaced by
--       UNIQUE comm_outbox_email_msgid_uniq over the same partial expression,
--       making a multi-parent Message-ID collision structurally impossible
--       going forward. If historical collisions exist the swap is skipped
--       (NOTICE with the count; the classifier already treats a multi-parent
--       match as identity_conflict, so routing stays correct either way).
--
-- Current-body pointers (plan §9.6.1 "findable from the latest migration"):
--   * capture_persona_inbound (canonical coordinator) — 20260722000002 §G
--   * _comm_resolve_parent_recipient                  — 20260722000003 §D
--   * _comm_outbox_snapshot_recipients                — 20260722000003 §E
--   * _comm_resolve_identity_claims                   — 20260722000002 §B
--   * complete_send                                   — THIS file (§B below)
--   * contract doc — docs/persona-email-contract.md
-- ============================================================================

-- ============================================================================
-- (A) Drop the dormant pre-v2 classifier
-- ============================================================================

drop function public._capture_persona_inbound_before_reply_recovery(
  uuid, text, text, text, text, text, text[], text[], text, text, jsonb,
  text, text, text[], text, text, text, timestamptz, text
);

-- ============================================================================
-- (B) complete_send — graceful duplicate-Message-ID handling
-- ============================================================================
-- Verbatim from the head (20260722000002 §J) except the one marked block:
-- the duplicate check runs after the row lock / status gates and BEFORE the
-- relay-leg branch, so BOTH stamping sites (the relay-leg update and the
-- journal-path update) see the degraded-to-null v_msgid.
...
-- ============================================================================
-- (C) Message-ID uniqueness — swap the lookup index for a UNIQUE one when clean
-- ============================================================================
-- Safe now because (B) degrades a duplicate to a NULL stamp before any insert
-- or update could collide. (The advance check leaves a narrow two-writer race
-- on concurrent completions of different rows; the unique index is exactly
-- the backstop that keeps even that race from minting a second parent —
-- routing already treats a multi-parent match as identity_conflict.)
...
begin
  ...
  if v_dupes > 0 then
    raise notice
      ...
      'keeping the NON-unique comm_outbox_email_msgid_idx — write a repair report, '
      'then re-attempt uniqueness in a later forward migration.',
      v_dupes;
  else
    drop index public.comm_outbox_email_msgid_idx;
