create function public._plan_rent_adjustment(p_account_id uuid,p_tenancy_id uuid,p_payload jsonb)
...
begin
  ...
  for s in select * from public.rent_schedules where account_id=p_account_id and tenancy_id=p_tenancy_id
    and deleted_at is null and kind='rent' order by start_date desc,id loop
    scope:=null;
...
create function public.preview_rent_adjustment(p_account_id uuid,p_tenancy_id uuid,p_payload jsonb)
returns jsonb language sql stable set search_path=public as $$
  select public._plan_rent_adjustment(p_account_id,p_tenancy_id,p_payload)-array['_segments','_plans','_selected_ids']
$$;
revoke all on function public._plan_rent_adjustment(uuid,uuid,jsonb),public.preview_rent_adjustment(uuid,uuid,jsonb)
  from public,anon,authenticated,service_role;
grant execute on function public._plan_rent_adjustment(uuid,uuid,jsonb),public.preview_rent_adjustment(uuid,uuid,jsonb)
  to authenticated,rent_adjustment_writer;
