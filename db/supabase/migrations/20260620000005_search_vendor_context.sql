-- ----------------------------------------------------------------------------
-- Entity search context, PR 3: vendor context (the agent's "the plumber we used
-- last time" lookup; see docs/search-context-enrichment.md).
--
-- CREATE OR REPLACE of search_entities only -- signature, return columns,
-- ranking, the 5 UNION matching branches and the tenant/area/property/
-- maintenance_request context branches are all unchanged from 20260620000004.
-- The only change is the per-type context CASE:
--
--   * vendor -- new: outreach `contact` (email, else phone) + `last_used_at`
--               (the most recent of completed_at/scheduled_for/created_at across
--               the vendor's work orders) + `job_count` (total work orders).
--               No `status` field -- soft-delete (deleted_at) already excludes
--               deactivated vendors. No `trade` field -- trade is a separate
--               categorization track, not part of this enrichment.
--
-- Every context object carries a `kind` discriminator. The vendor-history
-- aggregate uses `left join lateral ... on true` with an un-grouped aggregate,
-- so a vendor with no work orders yields job_count = 0 / last_used_at = null
-- (a row, not no-row). A new additive, idempotent index supports it:
--   work_orders (account_id, vendor_id).
-- Still computed only for the rows that survive ORDER BY ... LIMIT, and
-- SECURITY INVOKER so RLS scopes every read.
-- ----------------------------------------------------------------------------

create index if not exists work_orders_account_vendor_idx
  on public.work_orders (account_id, vendor_id);
