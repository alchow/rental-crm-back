...
-- ============================================================================
-- SCOPE DECISION (on record, review finding): the human link writes an
-- ACCOUNT-WIDE human_link claim and supersedes differing learned/legacy claims
-- across all scopes. For a genuinely shared inbox this means one human link
-- stops future cold mail from that address triaging for the other role — the
-- deliberate trade: the link UI offers no scope choice yet, and inventing a
-- silent scope the human didn't express would be worse. Follow-up: a scope
-- picker in the triage UI, then scoped links here.
-- ============================================================================
...
-- ============================================================================
-- (J) complete_send — claims-aware cast fallback (review fix)
-- ============================================================================
-- The live complete_send (head: 20260720000001) fell back to an unfiltered
-- channel_identities read for bare sends whose snapshot froze no party. Under
-- the multi-claim model that read could cast a SUPERSEDED or arbitrary claim.
-- Re-created verbatim from the head with ONE block changed: the fallback now
-- goes through _comm_resolve_identity_claims (live, scope-applicable, single
-- winning tier) and requires exactly one party; otherwise the row keeps the
-- 'unspecified' sentinel. Everything else is byte-identical to the head.
