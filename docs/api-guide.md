...
| `GET`    | `/leases/{id}` |                                                                                                                                                                                                                                                                                               |
| `PATCH`  | `/leases/{id}` | Any subset of `term_end`, `deposit_amount_cents`, `deposit_currency`, `document`, `status`. **Rent terms (`rent_amount_cents`/`rent_currency`) are immutable** — a rent change is a new instrument, not an edit; sending either returns 400 (see `POST /tenancies/{tenancyId}/rent-changes`). |
| `DELETE` | `/leases/{id}` | Soft-delete.                                                                                                                                                                                                                                                                                  |

### Notices

Served instruments attached to a tenancy — an entry notice, a rent-increase notice, a termination notice. A notice is a **record** of what was served and how (`served_at`/`served_method`/`body`/`document`). For a month-to-month tenancy it is the anchor a rent change hangs off (the fixed-term equivalent is a renewal lease).
