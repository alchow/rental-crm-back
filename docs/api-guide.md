...
Two fields describe what the instrument *is*, deliberately split (`notice_label` was named `notice_type` before migration 20260801000006 — the words are a label, not a type). `notice_label` is the landlord's **verbatim words** (1–100 chars) — never rewritten, and rendered as typed into the evidence-export PDF. `notice_class` is the nullable machine-readable functional class beside it (`rent_change` \| `written_warning` \| `cure_or_quit` \| `nonpayment_demand` \| `other`, all functional acts rather than statutory names) — the queryable half ("was a written warning served in the last 12 months?"), derived by the client from the exact canonical label the landlord committed; free text and out-of-app writers leave it null, and null is always legitimate. The class is never displayed or exported in place of the words.
