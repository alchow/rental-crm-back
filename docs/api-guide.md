...
Required on every mutating request under `/v1/accounts/*`. Generate a UUID per logical operation; reuse only on network-layer retries of the _exact same_ operation.
...
await authedFetch(`/v1/accounts/${accountId}/properties`, {
  ...
  body: JSON.stringify({
    name: '12 Maple St',
    address: { line1: '12 Maple St', city: 'Springfield', state: 'IL', zip: '62701' },
  }),
});
...
Contract **documents** attached to a tenancy. A tenancy never _requires_ a lease (handshake/month-to-month tenancies are valid). One tenancy can have multiple leases (renewals, addenda).

> **Modeling note:** a _tenancy_ is the fact of occupancy; a _lease_ is a document. Timeline and balance data live on the tenancy, not the lease.

| Method  | Path                   | Body                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`   | `/leases`              | Supports `?tenancy_id=`, `?status=` filters. **Voided leases are returned** with `voided_at`/`void_reason` set — filter them out client-side if you only want live contracts. Soft-deleted rows have never been returned by this list.                                                                                                                                                                                                                                                                                                                                                                                                        |
| `POST`  | `/leases`              | `tenancy_id`, `term_start`, `term_end`\|null, `rent_amount_cents` (≥0), `rent_currency`, `deposit_amount_cents` (≥0, optional), `deposit_currency` (optional), `document` (optional object), `status` ∈ `draft`/`active`/`expired`/`superseded`, `corrects_lease_id` (optional uuid — must name a **voided** lease of the same tenancy, else 400).                                                                                                                                                                                                                                                                                            |
| `GET`   | `/leases/{id}`         |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `PATCH` | `/leases/{id}`         | Any subset of `term_start`, `term_end`, `rent_amount_cents`, `rent_currency`, `deposit_amount_cents`, `deposit_currency`, `document`, `status` — **what is accepted depends on the lease's status** (table below). Re-sending an **unchanged** stored value is always tolerated (read-modify-write echo-back); only a differing value on a frozen field is rejected.                                                                                                                                                                                                                                                                          |
| `POST`  | `/leases/{id}/void`    | `void_reason` (required, non-blank, ≤500). Stamps `voided_at`/`void_reason` and returns the lease, which stays readable and listed. Returns 404 if the lease does not exist or is already voided, 409 `instrument_anchored` if a live (non-deleted) rent schedule cites it — ending the schedule does not release it; delete it (`DELETE /rent-schedules/{id}`, never-billed only) or use `replace`.                                                                                                                                                                                                                                                                                                                                             |
| `POST`  | `/leases/{id}/replace` | `void_reason` (as above) + `lease` (`term_start`, `term_end`\|null, `rent_amount_cents`, `rent_currency`, `deposit_amount_cents`, `deposit_currency`, `document` — no `tenancy_id` or `status`, both inherited from the lease being replaced). One transaction: voids this lease, creates the replacement with the same `tenancy_id` and `status` and `corrects_lease_id` pointing back, and re-points the live schedules that cited the old lease. Returns `{voided, replacement, repointed_schedule_ids}`. 409 `schedule_conflict` if an anchored schedule's rent differs from the replacement's — that is a rent change, not a correction. |

There is **no `DELETE /leases/{id}`** (it returns 404). A lease is a record: it is voided with a reason, never removed.

**Mutability by status.** The database enforces this on every write path, and rejections carry a machine-readable `code`.

| Field                                              | `draft`                           | `active` / `expired`                                                 | `superseded` | voided               |
| -------------------------------------------------- | --------------------------------- | -------------------------------------------------------------------- | ------------ | -------------------- |
| `term_start`, `rent_amount_cents`, `rent_currency` | editable                          | frozen — 409 `lease_executed`                                        | frozen       | frozen               |
| `term_end`, `deposit_*`, `document`                | editable                          | editable                                                             | frozen       | frozen               |
| `status`                                           | → `active`/`expired`/`superseded` | `active` → `expired`/`superseded`; backwards is 409 `lease_executed` | frozen       | frozen               |
| void (`POST .../void`)                             | allowed                           | allowed                                                              | allowed      | 404 (already voided) |

Voided and superseded are checked first, so every rejection on a voided lease is `lease_voided` and every rejection on a superseded one is `lease_superseded`, whichever field was sent.

Status only moves forward. To change the rent on an executed lease use `POST /tenancies/{tenancyId}/rent-changes` (a rent change is a new instrument, not an edit); to fix a mistake in one use `POST /leases/{id}/replace`. See ADR-0014 and `docs/lease-lifecycle-fe-reply.md` for the correction recipes.
...
Two fields describe what the instrument _is_, deliberately split (`notice_label` was named `notice_type` before migration 20260801000006 — the words are a label, not a type). `notice_label` is the landlord's **verbatim words** (1–100 chars) — never rewritten, and rendered as typed into the evidence-export PDF. `notice_class` is the nullable machine-readable functional class beside it (`rent_change` \| `written_warning` \| `cure_or_quit` \| `nonpayment_demand` — functional acts rather than statutory names — plus the `other` sentinel) — the queryable half ("was a written warning served in the last 12 months?"), derived by the client from the exact canonical label the landlord committed; free text and out-of-app writers leave it null, and null is always legitimate. The class is never displayed or exported in place of the words.

| Method   | Path            | Body                                                                                                                                                                                                                                     |
| -------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`    | `/notices`      | Supports `?tenancy_id=` and `?notice_class=` filters (the class filter never matches unclassed rows).                                                                                                                                    |
| `POST`   | `/notices`      | `tenancy_id` (required), `notice_label` (required, 1–100), `notice_class` (optional, nullable enum), `served_at` (optional ISO datetime), `served_method` (optional, 1–100), `body` (optional, max 10000), `document` (optional object). |
| `GET`    | `/notices/{id}` |                                                                                                                                                                                                                                          |
| `PATCH`  | `/notices/{id}` | Any subset of `notice_label`, `notice_class`, `served_at`, `served_method`, `body`, `document` — the correction window closes when the notice anchors a schedule (409).                                                                  |
| `DELETE` | `/notices/{id}` | Soft-delete.                                                                                                                                                                                                                             |

### Incidents

Evidence-grade records of tenant conduct (noise, unauthorized pet, damage, harassment…) anchored to a tenancy. An incident is testimony written in pen: `description` and `occurred_at` are frozen at the database once saved — corrections are appended as journal notes, never edits. Only classification and outcome (`category`, `resolved_at`, `resolution_note`) stay editable; dismissal is an audited soft delete. **Writes are owner/manager only, enforced at both the route guard and the RLS policy** — the agent principal and viewers can read but cannot author or alter incident records even by talking to the database directly. An incident _cites_ existing records through items — journal entries, maintenance requests, notices, inspections — exactly one per item; citations can be soft-unlinked but never edited or hard-deleted, and a journal entry cited by a live item has its testimony fields frozen too (retracting a cited journal entry is blocked with the citing incident's id in the error — unlink that citation first, then retract). Photos attach directly via `entity_type=incidents` on the attachments API — and uploading or deleting incident photos is owner/manager only, like every other incident write. Tenancy-scoped evidence exports render an Incidents section automatically, including incident photo custody. Note the two recurrence bases: the API's `recurrence` endpoint counts a trailing window measured back from **now** (the "where do we stand today" number), while the evidence export's ordinal ("3rd same-category incident…") anchors each incident's window at its own `occurred_at` — the same tenancy can legitimately show different counts in the app and the PDF.
...
Define a recurring charge. You cannot PATCH a schedule's **billing terms** — to change rent, end the current schedule and start a new one (or use the rent-changes verb below). PATCH exists for the **late-fee policy alone** (`grace_days`, `late_fee_cents`), which describes the lease rather than what was billed and from which nothing is _generated_. It stays editable even once fees exist: a fee already asserted is its own charge row carrying its own recorded amount, so changing the policy neither re-prices nor voids it. DELETE exists for one purpose: removing a **never-billed** mistaken schedule (the ADR-0012 corrections path). Creating a schedule **is** the instruction to bill: automatic charge generation is on by default and is **opt-out per account** (see the note below).

> **The late-fee policy is proposed, never charged.** `grace_days` (0–30) and `late_fee_cents` (>0) record what the lease says, so a client can tell a landlord "rent is late as of today — add the $85 fee?". Nothing server-side reads them: no cron, no generator, no trigger mints a `late_fee` charge. The fee becomes real only when a human confirms and the client POSTs an ordinary charge with `parent_charge_id` set to the rent charge. Both fields are **nullable with no default** — `null` means the lease terms were never recorded, and the client proposes nothing rather than guessing. There is deliberately **no timezone column**: because the assertion is a human tap, no server-side clock decision about "when the window closed" exists to get wrong.

| Method | Path                                  | Body / Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------ | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`  | `/rent-schedules`                     | Supports `?tenancy_id=` filter.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `POST` | `/rent-schedules`                     | `tenancy_id` (required), `kind` (required, 1–50), `amount_cents` (≥0), `currency`, `due_day` (1–28), `start_date`, `end_date`\|null, `grace_days`\|null (0–30), `late_fee_cents`\|null (>0).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `PATCH` | `/rent-schedules/{id}`               | **Late-fee policy only:** `grace_days`\|null (0–30) and/or `late_fee_cents`\|null (>0). Send `null` to clear a field back to "not set"; omit a field to leave it alone; an empty body is `400`. Any **other** key (`amount_cents`, `due_day`, `kind`, `start_date`, `end_date`) is rejected `400` rather than silently ignored — billing terms are immutable, a rent change owns the amount and `/end` owns the bound. Member-level, like every other schedule mutation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `GET`  | `/rent-schedules/{id}`                |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `POST` | `/rent-schedules/{id}/end`            | `end_date` (required, nullable). Sets `end_date`; the schedule stops generating charges after that date. `end_date: null` **re-opens** the schedule — for cancelling a planned end, or undoing a rent change that voided nothing (`voided_charge_ids: []`). Never re-open to undo a change that **voided advance charges**: a voided (schedule, period) pair never re-bills under the same schedule id, so the re-opened schedule silently skips those periods — end that undo with a fresh **continuation schedule** instead (`POST /rent-schedules`, old terms, `start_date` = the mistaken effective date; the new id re-bills on the next generator run **while the voided period's due day is still ahead** — a later undo re-creates the elapsed period manually with the continuation's provenance).                                                                                                                                    |
| `DELETE` | `/rent-schedules/{id}`              | Soft-delete a **never-billed** schedule (corrections path; resolves the rent-change `schedule_conflict` 409 on a mistaken future era). Refused `409 schedule_has_charges` while non-voided charges reference it — void those first (`POST /charges/{id}/void`); a billed era is ended, never deleted. Deleting a schedule releases the write-block on the lease/notice that anchored it.                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `POST` | `/tenancies/{tenancyId}/rent-changes` | Instrument-anchored rent change. `amount_cents` (≥0), `currency`, `effective_date` (YYYY-MM-DD), `due_day` (1–28, optional), `change_reason` (optional, 1–2000), `kind` (optional, defaults `rent`), and **at least one anchor** — `source_lease_id` (fixed-term renewal) and/or `source_notice_id` (month-to-month notice; both together = a renewal that followed a notice). Atomically ends the open same-kind schedule at `effective_date−1`, inserts the successor schedule (with provenance back to the anchor), and — when lease-anchored — supersedes the prior active (non-voided) lease(s) and activates a `draft` anchor lease. Any charges the generator had already advance-created off the old era for periods on/after `effective_date` are **voided automatically** (the successor era re-bills them at the new amount). Returns `{rent_schedule, ended_schedule_ids[], superseded_lease_ids[], voided_charge_ids[]}`. Voided periods are re-billed by the **next daily generator run** (auto-charge accounts only), not synchronously — and **only while the period's due day is still ahead** (the generator never backfills): a **backdated** change (applied after the effective period's due day) leaves that period with no live charge; re-create it manually via `POST /charges` at the new amount with `source_schedule_id` = the successor. Conflicts carry fine-grained codes: `tenancy_ended`, `notice_not_served` (serve it first), `instrument_not_current` (expired/superseded/voided anchor), `schedule_conflict` (a same-kind schedule starts on/after `effective_date` — delete it if mistaken, or pick a later date). The successor **inherits the ended schedule's late-fee policy**; pass `grace_days` / `late_fee_cents` to set different values for the new era (a rent increase never silently drops the lease's fee terms). Neither can _clear_ a policy — PATCH the successor with `null` for that. |

Lease corrections and genuine rent changes share one reviewed command:

| Method | Path | Behavior |
| --- | --- | --- |
| `POST` | `/tenancies/{tenancyId}/rent-adjustments/preview` | Read-only preview for `edit_details`, `correct_rent`, or `change_rent`. Returns server-calculated lease, schedule, bill, payment-application, credit, and balance effects plus blockers and an opaque `preview_token`. This route is the one account-scoped POST that does not require `Idempotency-Key`. |
| `POST` | `/tenancies/{tenancyId}/rent-adjustments` | Requires `Idempotency-Key` and `{input, preview_token}`. Replans under lock; a changed source, bill, payment, or schedule returns `409 preview_stale` without writes. Success atomically stores and returns the immutable receipt, including created source instrument IDs for follow-up document uploads. |
| `GET` | `/tenancies/{tenancyId}/rent-adjustments` | Lists immutable receipts newest first. Supports opaque `cursor` pagination and `limit` 1–100 (default 50); returns `{items, next_cursor}`. |
| `GET` | `/tenancies/{tenancyId}/rent-adjustments/{adjustmentId}` | Reads one account-authorized immutable receipt. |
| `GET` | `/tenancies/{tenancyId}/rent-adjustments/by-request-key/{requestKey}` | Resolves an uncertain save as `{receipt}`. `receipt: null` means no durable adjustment exists yet; retry the exact commit body with the same key. |

> **Changing the rent is anchored to an instrument, never a free-floating edit.** A lease's `rent_amount_cents` is immutable and you cannot PATCH a schedule's amount; instead POST a rent change anchored to the renewal lease (fixed-term) or the served notice (month-to-month). The one call ends the current schedule at `effective_date−1`, opens the successor with a link back to that anchor, and supersedes the old lease — so the history stays honest about what the rent was, when it changed, and why. The daily charge generator additionally logs a `rent_drift_detected` warning if an active lease's rent ever diverges from its open schedules.

> **Automatic charge generation is on by default — opt out to disable it.** The API process runs the `rent-charges` job once daily at 08:00 UTC (`api/src/admin/scheduled-jobs.ts`; `SCHEDULED_JOBS_ENABLED` is the kill switch). It generates charges for accounts where `auto_charge_enabled = true`, which is the **default for every account** since 2026-08-01 (ADR-0011 amendment; migration `20260801000001_auto_charge_default_on` also backfilled the accounts that predated it). Turn it **off** for an account with `PATCH /accounts/{accountId}/settings { "auto_charge_enabled": false }` (owner/manager only); read the current value with `GET` on the same path. The generator only ever mints a charge where a **live rent schedule covers the period**, so an account with the flag on but no schedules is billed nothing. For an account it does bill, it creates **next period's** charge the day _after_ the current period's due date, so the tenant has the invoice in hand before it is due; it is idempotent (a missed or retried run never double-bills) and it **never backfills** past periods. Accounts that have opted out are never touched. If you prefer to create charge rows yourself, opt out and do so via `POST /charges` at any time — a schedule-generated and a manually created charge are identical, both plain `Charge` rows.

### Charges

Amounts owed. No mutation — corrections use `void` plus a new reversing entry (e.g., charge an `nsf_fee` after a bounced check, don't edit the original payment). One charge per `(source_schedule_id, period_start)`, **voided rows included** (the cron-idempotency key): a create naming an occupied schedule+period is a 409 `conflict`; to re-bill such a period manually, omit `source_schedule_id`.

| Method | Path                 | Body / Notes                                                                                                                      |
| ------ | -------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `GET`  | `/charges`           | Supports `?tenancy_id=` filter.                                                                                                   |
| `POST` | `/charges`           | `tenancy_id`, `type`, `amount_cents` (>0), `currency`, `due_date`, `period_start`\|null, `period_end`\|null, `description`\|null, `parent_charge_id` (optional). |
| `GET`  | `/charges/{id}`      |                                                                                                                                   |
...
Voiding a **parent by hand** deliberately does **not** cascade: a late fee already asserted against it stays live and must be voided on its own (`POST /charges/{id}/void`). Withdrawing a fee the landlord asserted is their decision, not a side effect of ours — both rows stay visible in the ledger with their own void reasons. A **rent change** is the one exception, and only because it is the system unwinding its own generated charge: `POST /tenancies/{tenancyId}/rent-changes` voids the old era's advance rent charges and, with them, the live charges whose `parent_charge_id` names one (`void_reason` `"parent charge superseded by rent change"`). All of those ids come back in `voided_charge_ids`. Without that cascade a **backdated** change would strand a live fee against a voided bill, and re-creating that period's rent charge manually would let a *second* live fee be asserted against the new parent.

When you re-assert a fee after voiding one, send a **fresh `Idempotency-Key`**. The idempotency cache stores 4xx outcomes as well as successes, so replaying the key that earned the original `409 late_fee_exists` replays that stale `409` instead of creating the corrected fee.
...
| `PATCH` | `/tenancies/{tenancyId}/adoption`   | `{needs_review}` only — resolves the "saved as unresolved" flag. Every other adoption field is frozen testimony at the DB (trigger-enforced); corrections beyond the flag are an operator soft-delete + re-adoption. |

Body: `adoption_date` (not in the future; one extra day tolerated for client timezones ahead of UTC), `currency`, `rent` (`amount_cents`, `due_day` 1–28, `start_date` — must be on or after the tenancy's recorded `start_date`, else 409 `tenancy_start_date_conflict`: correct the tenancy first, which is only possible while its timeline is virgin), then **either** itemized history — `charges[]` (`amount_cents`, `due_date`, ≤120; the billing **period is derived server-side and snapped to the schedule's due-day grid** — the grid window containing `due_date` — while `due_date` stays your verbatim date, so backfilled periods always sit on the exact grid the generator dedupes on; two charges landing in one billing period are rejected) and `payments[]` (`amount_cents`, ISO `received_at` — bounded to `adoption_date` with a one-day UTC slack for same-day western-timezone receipts, and no earlier than one month before the schedule start (year-typo guard), `method`, optional `allocations[]` naming charges by `charge_index`, ≤24 per payment, no duplicate index, ≤200 payments) — **or** a signed `opening_balance_cents` (> 0 owed, < 0 credit; optional `balance_basis`, `needs_review`). The two are mutually exclusive; an optional held `deposit` (`amount_cents`, `received_on`; its payment is recorded at noon UTC so it renders the entered day in every timezone) is allowed with both. Returns `{adoption_id, schedule_id, charge_ids[], payment_ids[], deposit_charge_id}` with ids in request order. A payment inside the one-day slack appears in `?as_of` snapshots from its UTC date onward — the platform-wide snapshot rule. The virgin-timeline and future-date invariants are also DB-trigger-enforced against direct writes, and adoption facts freeze after commit (only `needs_review` and the audited soft-delete may change).
...
```jsonc
{
  ...
  "entries": [
    ...
    },
    {
      ...
      "is_deposit": false,
      "voided_at": null,
      "parent_charge_id": null, // set on a derived charge, e.g. a late_fee's rent charge
    },
    {
      "kind": "payment",
      "id": "pay_1",
      "occurred_at": "2026-06-01", // received_at — when the money ARRIVED
      "created_at": "2026-06-03T18:22:11Z", // when it was RECORDED; never render as the payment date
      "amount_cents": 70000,
      "method": "check",
      "reference": null,
      "voided_at": null,
      "allocations": [
        { "charge_id": "c_1", "amount_cents": 70000, "created_at": "2026-06-03T18:22:11Z" },
      ],
    },
  ],
  "totals": {
...
| Method  | Path                         | Body                                                                                                     |
| ------- | ---------------------------- | -------------------------------------------------------------------------------------------------------- |
| `GET`   | `/maintenance-requests`      | Supports `?area_id=` and `?status=` filters. `status` accepts a single value **or** a comma-separated set (e.g. `?status=open,triaged`); an unknown member is a 400 with `error.details.fieldErrors.status`. |
...
| Method | Path                 | Body                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------ | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`  | `/interactions`      | Query filters: `tenancy_id`, `maintenance_request_id`, `area_id`, `direction`, `party_type`, `party_id`, and `latest_only=true` (returns only chain heads — the collapsed view; default returns the full set so chains can be reconstructed). **`party_id`** finds every interaction that involves one person: a row matches when the **cast** (`interaction_participants`) names them **or** the row's own party fields do — so a group message or witnessed exchange where they were one of several participants matches, and so do castless entries such as a party-carrying `note` or a correction head that inherited the party from the entry it corrects. Add `party_type` to narrow the matched leg to that person's tenant-vs-vendor role. **Caveat:** with `latest_only=true`, someone named _only_ in a superseded entry's cast (e.g. a cc on a corrected message) does not match — the correction head inherits just the headline party; omit `latest_only` for the complete involving-them set. |
...
`party_type` values: `tenant` / `vendor` / `inspector` / `other` / `unspecified` / `none`. `none` is forbidden on communications (DB-enforced as of `20260801000003`) — it is the default party-less shape for a `note`, which MAY instead carry a concrete role (`tenant`/`vendor`/`inspector`/`other`) plus a `party_id`. `unspecified` is the role-unknown capture sentinel — see below.
...
The target must be the **head** of its chain — correcting an already-superseded entry, or amending/retracting a retracted entry, returns `409 invalid_correction_target`; a `corrects_id` in another account returns `404`. A correction's `occurred_at` defaults to the original's (same event, same timeline position). Reads return two derived fields: `superseded_by_id` (the entry that corrects this one, or null) and `is_head`. A `note` is a dated observation that MAY name a counterparty ("spoke to Gina about the boiler") or none at all ("inspected roof, cracked tile"). Minimal recipe: `kind: "note"` with `occurred_at` and `body`; omit `channel`/`direction`. To name someone, add `party_type` (a concrete role — never `unspecified`) with `party_id` and/or `party_label`; the note is then discoverable via `?party_id=`.
...
**Email (slice 1, send-only)** rides the same outbox pipeline. An email intent may carry a **`subject`** (email-only; 400 on any other channel), frozen at intent time; on completion the journal records the full content as `Subject: <subject>` + blank line + body — one documented shape shared with the transport's template rendering. A third provenance class exists for core-originated transactional sends: `approval_ref='system:<flow>'` + `author_type='system'` (e.g. `system:capture_renewal`, the inspection-capture renewal email) — valid ONLY from core's own service tier; any API caller citing a `system:` ref is refused (403) and the DB rejects the forge. Unsubscribes are CAN-SPAM / RFC 8058 compliant: the transport mints stateless per-address HMAC links to the public `/v1/unsubscribe/email/{token}` endpoint (GET registers the opt-out and shows a confirmation page; POST is the Gmail/Yahoo one-click), which records a global `email` opt-out — parking queued intents and refusing new ones at 422 like every other channel.

**Email threads (slice 2)** relay natively in BOTH directions with no shared-address ambiguity: at creation, every participant — the landlord included (a `landlord_user` participant with an email is required; they reply from their own inbox) — gets a UNIQUE tokenized reply address `t-<token>@<EMAIL_REPLY_DOMAIN>` on their binding (`reply_address`). Inbound email routes by that token, never by content or sender; the sender address is a **verification** input — a resolved token whose from-address is not the bound participant's journals into the thread with `party_type='unspecified'` and the actual sender as the label, and returns `disposition='sender_mismatch'` (the transport must not auto-relay it as verified). Unknown token → `orphan`. Threads store their `channel` (frozen) and an optional email-only `subject` seed the transport continues as "Re: …"; thread-message legs dial on the thread's channel and carry no outbox subject of their own. Group mode stays sms-only.

Threads carry a **`mode`**: `bridged` (default — each counterparty has a private 1:1 leg on the platform number and core relays between them) or `group` — one provider-native group-MMS session: the platform number plus 2–7 distinct member addresses (8 participants including our number), sms-only, and the landlord's own phone **is** a group member (a `landlord_user` participant with an address is required and gets bound like everyone else). No two active group threads on one number may carry the identical member set. Group sends have no relay concept and no per-recipient legs: a send into a group thread is **one** outbox row with `to_address` null and the frozen recipient set in `group_addresses`; the provider's `group_message_id` becomes its `provider_sid` at completion, and the journal gains exactly one entry. Inbound group messages carry the other recipients in `cc`; routing is by participant-**set** match (`{from} ∪ cc` minus our number == a thread's member set; no match → `orphan`), while 1:1 messages (no `cc`) keep the per-address binding routing — both modes coexist on one number. STOP compliance is set-wide: an opt-out by **any** member refuses new group intents (422 `opted_out`) and parks queued group rows to that thread; inbound is still captured (the contact is evidence).

**Evidence hardening (EV-A/EV-B).** Every journal row can carry a **cast** — `participants[]`, one entry per person-per-role (`sender`/`recipient`/`cc` on the wire, `attendee` in person), each with the wire address, a typed party reference resolved at write time, and a display-name snapshot — stamped server-side by the comms capture/completion paths (from the frozen intent, never transport input) or written atomically with a manual entry via `POST /interactions` `participants[]` (landlord-only; 1–20 entries; fresh `kind='communication'` only). Even a hand-logged communication that supplies **no** `participants[]` but names a single counterparty in the legacy slot (`party_id`, or `party_label` under a `tenant`/`vendor`/`inspector`/`other` `party_type`) is automatically given a **one-person cast** (role derived from `direction`: inbound→sender, outbound→recipient, else attendee) so it is discoverable by the `?party_id=` filter — every communication involving a person is one indexed cast query. (Agent-authored communications keep their explicit transport-written cast; the `unspecified` unresolved-sender queue and `import`-channel rows stay on the legacy path.) The cast is member-readable, hard insert-only (client writes are revoked and updates/deletes raise at the DB), and belongs to the root entry of a correction chain; outbound casts are copied from a recipient snapshot frozen at intent time. Alongside it, every row states its trust tier in **`attestation`**: `provider_verified` (carrier-confirmed transmission; DB-gated to the verified comms paths, unforgeable elsewhere), `attested` (a human's account of an off-platform event — the DB default for every new manual entry, cast or castless), `imported` (import-channel rows), or `null` (strictly pre-migration legacy). The legacy `party_*` fields remain populated as the derived single-slot headline. The cast is the richer record — it is the only place a multi-party exchange is described per person-per-role — but discovery no longer depends on it: `?party_id=` matches the cast **or** the headline slot, so castless shapes (a party-carrying `note`, a correction head that inherited the slot) are found too. Alongside it, the transport archives the **verbatim signed webhook** for every inbound message via `POST /comms/evidence` (archive-then-process, BEFORE parsing; see `docs/comms-evidence.md`): the body hash is computed server-side, recorded on an audit-anchored `inbound_provenance` row (the hash lands in the per-account event hash chain), and the exact bytes live in the private `comm-evidence` bucket — so a provider signature can be re-verified independently of our software years later. Provenance rows are never deleted; the retention janitor (default ≈7 years, `COMM_EVIDENCE_RETENTION_DAYS`) removes only blobs, as an audited event. A per-account **legal hold** (`PUT /comms/legal-hold`, owner/manager only) suspends every comms destruction path — the raw-capture prune and the evidence-blob retention — while active.

| Method | Path                              | Body / Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------ | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST` | `/comms/outbox`                   | Create a send intent (`queued`). Transport + landlord. `channel`, `body`, `approval_ref`, and either `to_address` or `thread_id`+`participant_ref` — or just `thread_id` for a **group** thread (recipients derive from its bindings; `to_address`/`participant_ref`/relays are rejected there). Optional `subject` (email only), `tenancy_id` / `maintenance_request_id` context, `relay_of_interaction_id` for relays. 422 `opted_out` if the address (any group member) opted out. |
| `GET`  | `/comms/outbox`                   | List outbox rows — the transport dispatch scan **and** the landlord send-state read (inspection email chips, statement rent-nudge trail). Transport + landlord, same page and filters for both: `status`, `channel`, `eligible_at` (honours `not_before`). Cursor-paginated.                                                                                                                                                                                                          |
...
| `POST` | `/comms/evidence`                 | Archive the verbatim signed webhook for an inbound message (transport). Server-side sha256, audit-anchored provenance row, bytes to the private evidence bucket. Idempotent on `provider_msg_id`; a replay with a **different** body is refused (409) — first archived claim wins.                                                                                                                                                                                                    |
| `GET`  | `/comms/legal-hold`               | Read the account legal-hold state (any member). `active=false` with null timestamps = never set.                                                                                                                                                                                                                                                                                                                                                                                      |
| `PUT`  | `/comms/legal-hold`               | Set or release the legal hold (owner/manager; the agent principal is denied). While active, the raw-capture prune and evidence-blob retention skip this account. Audited.                                                                                                                                                                                                                                                                                                             |
...
| `GET`  | `/comms/threads/{id}`             | Thread detail (transport + landlord): participants, channel bindings, and journal messages with derived delivery state (cursor/limit page the messages). The transport reads it for relay-leg and thread-send addressing.                                                                                                                                                                                                                                                                                                                                                     |
| `POST` | `/comms/threads`                  | Create a thread + participants + bindings (landlord). Optional `mode: 'group'`, `channel: 'email'` (+ email-only `subject` seed; landlord participant with an email required; per-participant reply tokens minted). Bridged sms: one active thread per counterparty per platform number. Group: one active thread per member **set** per platform number (409 on an identical set), and the landlord leg must be the **caller's own OTP-verified phone** (409 if unverified, mismatched, or a different user).                                                                                                   |
...
| `GET`  | `/comms/policies`                 | List standing grants (transport + landlord). The transport reads active grants for `grant:<id>` send provenance.                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `GET`  | `/comms/platform-numbers`         | List the account's provider numbers (transport + landlord). The transport routes inbound by them and picks the outbound From from them; the landlord reads them to know whether texting is on. Filter `status=active` for routing — a released number must not resolve to its former account.                                                                                                                                                                                          |
| `POST` | `/comms/platform-numbers`         | Register a carrier-ordered number (**transport only**). Idempotent on (account, number) so a replayed provisioning step is a no-op; re-activates a released number. A number held by another account is a 409, never a reassignment. Members hold SELECT-only on the table, so this RPC-backed route is the only write door.                                                                                                                                                            |
...
An sms intent carries `platform_number`: the account's own number the send must dial FROM, frozen at intent time from the thread binding (or, for a bare send, the account's active sms-capable number). The transport dials exactly that and `complete_send` journals exactly that, so a record can never name a sender the carrier did not see. Null on email/voice, and on sms rows queued before an account is provisioned — the transport fails those terminally as `no_platform_number` rather than falling back to another number.
...
await post(`/v1/accounts/${accountId}/inspections/${ins.id}/items`, {
  label: 'Kitchen — sink',
  condition: 'Good',
  notes: 'Minor scratch on left basin',
});
await post(`/v1/accounts/${accountId}/inspections/${ins.id}/items`, {
  label: 'Bedroom — carpet',
  condition: 'Fair',
  notes: 'Stain in NW corner',
});
...
**Deduplication:** if an open request already exists for the same `area_id` + `title`, the submission merges into it (a new interaction + photo are added; a new request is _not_ created). `deduped_onto_existing: true` signals this.
...
const { data, error } = await api.GET('/v1/accounts/{accountId}/tenancies/{tenancyId}/ledger', {
...
// 4. Rent schedule. This starts automatic billing: auto_charge_enabled
//    defaults to true. Opt out with
//    PATCH /accounts/{accountId}/settings { auto_charge_enabled: false }
...
const token = await post(`/v1/accounts/${accountId}/intake-tokens`, { tenancy_id: tenancy.id });
...
_(Each `post`/`patch`/`put` call sets `Authorization` and a fresh `Idempotency-Key`.)_
...
Every consequential action is recorded as an immutable, server-timestamped, hash-chained audit event. Records are _corrected_ by new entries — never silently overwritten. The sequence of events is verifiable: `chain_verified: true` in an evidence export means the chain hashes out cleanly from the first event to the last.
...
Money history — individual **charges and payments** — is **out of scope** in v1 and cannot be imported (it's deferred to a later phase). `rent_schedule` captures the _recurring rent amount_, which is structural, not a charge.
