...
> **Automatic charge generation is opt-in.** A Render cron service (`rent-charge-generator` in `render.yaml`) runs the `charges:generate` script once daily. It generates charges **only for accounts that have set `auto_charge_enabled = true`** — flip it with `PATCH /accounts/{accountId}/settings` (owner/manager only). For an opted-in account it creates **next period's** charge the day _after_ the current period's due date, so the tenant has the invoice in hand before it is due; it is idempotent (a missed or retried run never double-bills). Accounts that have not opted in are never touched. If you prefer to create charge rows yourself, do so via `POST /charges` at any time — a schedule-generated and a manually created charge are identical, both plain `Charge` rows.
