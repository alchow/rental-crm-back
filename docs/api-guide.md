...
| `GET`    | `/leases/{id}` |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `PATCH`  | `/leases/{id}` | Any subset of `term_end`, `deposit_amount_cents`, `deposit_currency`, `document`, `status`. **Rent terms (`rent_amount_cents`/`rent_currency`) are immutable** — a rent change is a new instrument, not an edit; sending a **changed** value returns 400 (see `POST /tenancies/{tenancyId}/rent-changes`). Re-sending the **unchanged** stored values (a read-modify-write echo-back) is tolerated. A `superseded` lease is a historical record — transitioning it back out of `superseded` returns 409. |
