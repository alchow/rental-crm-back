...
export interface paths {
    ...
    "/v1/accounts/{accountId}/comms/outbox": {
         ...
         * @description Both principals get the identical page shape, filters, and cursor pagination, scoped to the path account by the membership guard and RLS. Two landlord surfaces need the LIST rather than the single-row read because they resolve send state across many rows at once: the inspection email send-state chips (matching template_id-tagged rows) and the statement rent-nudge trail. This endpoint refused every non-agent principal until now, so those landlord reads returned 403 and rendered as "never sent" in production. Owner/manager read it under the same principal rule as GET /comms/outbox/{id}; viewers stay denied, and the dispatch/reconcile MUTATIONS remain transport-only.
         */
        get: {
            parameters: {
                query?: {
